            ...
            <div class="card">
                <div class="card-body">
				 <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data" >
                    <h3 class="text-center bg-warning text-white p-3">IDENTITY CARD REPORT </h3>			
                    <?php echo $msg;?>
                    <table class="table table-hover table-bordered table-striped">
                        ...
                        <?php
                            $sql="SELECT * FROM sammelen_invoice WHERE status='2' and created_by='".$_SESSION['username']."'";
                            $res=execute_query($sql);
                            $totalmember=0;
                                ...
                                while($row=mysqli_fetch_assoc($res)){
                                    ...
                                    ?>
                                        <td><a href="sammelen_reg_view.php?id=<?php echo $row['sno']?>" class="btn btn-warning btn-sm" target="_blank">View</a></td>
										 <td><a href="" onClick="confirm('Are you sure?')" class="btn btn-success ">Print ID Card</a></td>
