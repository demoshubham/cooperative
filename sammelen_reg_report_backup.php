...
if(isset($_GET['delid'])){
    ...
    $sql="UPDATE sahkariya_sammelen_invoice SET status='0' WHERE sno='{$_GET['delid']}'";
    $res=execute_query($sql);
    if(mysqli_error($db)){ 
        $msg .= '<p class="alert alert-danger">Error # 1.01 : '.mysqli_error($db).'>> '.$sql.'</p>';
    }
    // $sql="DELETE FROM sahkarita_sammelen_trans_sanchalak WHERE sahkariya_sammelen_invoice_id='{$_GET['delid']}'";
    // $res=execute_query($sql);
    // if(mysqli_error($db)){ 
    //     $msg .= '<p class="alert alert-danger">Error # 1.01 : '.mysqli_error($db).'>> '.$sql.'</p>';
    // }
    // $sql="DELETE FROM sahkarita_sammelen_trans_sanstha WHERE sahkariya_sammelen_invoice_id='{$_GET['delid']}'";
    // $res=execute_query($sql);
    // if(mysqli_error($db)){ 
    //     $msg .= '<p class="alert alert-danger">Error # 1.01 : '.mysqli_error($db).'>> '.$sql.'</p>';
    // }
    // $sql="DELETE FROM sahkarita_sammelen_trans_new_sadsaya WHERE sahkariya_sammelen_invoice_id='{$_GET['delid']}'";
    // $res=execute_query($sql);
    // if(mysqli_error($db)){ 
    //     $msg .= '<p class="alert alert-danger">Error # 1.01 : '.mysqli_error($db).'>> '.$sql.'</p>';
    // }
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table table-hover table-bordered table-striped">
                        ...
                        <?php
                            $sql="SELECT * FROM sahkariya_sammelen_invoice WHERE status='1'";
                            $res=execute_query($sql);
                            $totalmember=0;
