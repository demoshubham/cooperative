...
elseif($id=='otp'){
	...
	if(isset($_POST['user_role'])){
		switch($_POST['user_role']){
			case 'dr':{
				$sql = '(SELECT sno, ar_name as full_name, mobile_number, "ar_dr" as info FROM `ar_dr` where mobile_number="'.$_POST['number'].'")';
				break;
			}
			case 'ar':{
				$sql = '(SELECT sno, ar_name as full_name, mobile_number, "ar" as info FROM `ar` where mobile_number="'.$_POST['number'].'")';
				break;
			}
			case 'adco':{
				$sql = ' (SELECT sno, adco_name as full_name, mobile_number, "adco" as info FROM `adco` where mobile_number="'.$_POST['number'].'")';
				break;
			}
			case 'ado':{
			$sql = '(SELECT sno, ado_name as full_name, mobile_number, "ado" as info FROM `ado` where mobile_number="'.$_POST['number'].'")';
				break;
			}
		}
	}
		...
		$sql = '
		(SELECT sno, ar_name as full_name, mobile_number, "ar_dr" as info FROM `ar_dr` where mobile_number="'.$_POST['number'].'")
		union all
		(SELECT sno, ar_name as full_name, mobile_number, "ar" as info FROM `ar` where mobile_number="'.$_POST['number'].'") 
		union all 
		(SELECT sno, ado_name as full_name, mobile_number, "ado" as info FROM `ado` where mobile_number="'.$_POST['number'].'") 
		union all 
		(SELECT sno, adco_name as full_name, mobile_number, "adco" as info FROM `adco` where mobile_number="'.$_POST['number'].'")';
	...
	elseif(mysqli_num_rows($result)>1){
		$data[0] = array("status"=>"multi", "msg"=>"Multiple User Roles Found. Please select one.");
		$data[0]['opt_val'] = array();
		while($row = mysqli_fetch_assoc($result)){
			switch($row['info']){
				case 'ar_dr':{
					$data[0]['opt_val'][$row['info']] = 'DR';
					break;
				}
				case 'ar':{
					$data[0]['opt_val'][$row['info']] = 'AC &amp; AR';
					break;
				}
				case 'ado':{
					$data[0]['opt_val'][$row['info']] = 'ADO';
					break;
				}
				case 'adco':{
					$data[0]['opt_val'][$row['info']] = 'ADCO';
					break;
				}
			}
		...
		$sql = "insert into session (user_type, user_id, otp_verification, s_start_date, s_start_time, last_active, s_id) values ('".$row['info']."', '".$row['sno']."', '".$otp."', '".date("Y-m-d")."','".date("H:i:s")."', '".time()."', '".$_SESSION['session_id']."')";
		execute_query($sql);
		$data[] = array("status"=>"otp_sent", "msg"=>"OTP Sent on mobile.");
		$template_id = '1207166661218817759';
			$pe_id = $peID;
...
elseif($id=='verifyotp'){
	$msg='';
	//print_r($_POST);
	$sql = 'select * from session where s_id="'.$_SESSION['session_id'].'" and user_type="'.$_SESSION['user_type'].'" and user_id="'.$_SESSION['user_id'].'"';
	//echo $sql;
	$result = execute_query($sql);
	if(mysqli_num_rows($result)!=0){
		$row = mysqli_fetch_assoc($result);
		if($row['otp_verification']==$_POST['otp']){
			...
			switch($_SESSION['user_type']){
				case 'ar':{
					$sql = 'select * from ar where sno='.$_SESSION['user_id'];
					break;
				}
			}
			//echo mysqli_error($db).$sql;
...
elseif($id=='send_verify_otp'){
	//print_r($_POST);
	$otp = randomnumber();
	
	$sql = 'select * from survey_invoice_validation where survey_id="'.$_SESSION['survey_id'].'" and user_id="'.$_SESSION['user_id'].'" and user_type="'.$_SESSION['user_type'].'" and approval_status="approve" order by sno desc limit 1';
	$result = execute_query($sql);
	if(mysqli_num_rows($result)!=0){
		$row = mysqli_fetch_assoc($result);
		if($row['otp_verify']=='1'){
			...
			$sql = 'update survey_invoice_validation set otp_verify="'.$otp.'" where sno='.$row['sno'];
			execute_query($sql);	
			$data[] = array("status"=>"otp_sent", "msg"=>"OTP Sent on mobile.");
			$template_id = '1207166661218817759';
			$pe_id = $peID;
...
Regards,
WeKnow Technologies';
			send_sms($row['mobile_number'],$get_msg, $template_id, $pe_id, $hindi='');
		...
		switch($_SESSION['user_type']){
			case 'ar':{
				$sql = 'select * from survey_invoice_validation where survey_id="'.$_SESSION['survey_id'].'" and approval_status="approve" and user_type="secretary" and status="3"';
				$row_request = mysqli_fetch_assoc(execute_query($sql));

				$sql = 'select sno, ar_name as full_name, mobile_number from ar where sno='.$_SESSION['user_id'];
				break;
			}
			case 'adco':{
				$sql = 'select * from survey_invoice_validation where survey_id="'.$_SESSION['survey_id'].'" and approval_status="approve" and user_type="secretary" and status="2"';
				$row_request = mysqli_fetch_assoc(execute_query($sql));

				$sql = 'select sno, adco_name as full_name, mobile_number from adco where sno='.$_SESSION['user_id'];
				break;
			}
			case 'ado':{
				$sql = 'select * from survey_invoice_validation where survey_id="'.$_SESSION['survey_id'].'" and approval_status="approve" and user_type="secretary" and status="1"';
				$row_request = mysqli_fetch_assoc(execute_query($sql));

				$sql = 'select sno, ado_name as full_name, mobile_number from ado where sno='.$_SESSION['user_id'];
				break;
			}
		}
		//echo $sql;
		$user = mysqli_fetch_assoc(execute_query($sql));

		$sql = 'insert into survey_invoice_validation (survey_id, request_id, user_id, user_type, mobile_number, otp_verify, `ip_address`, `http_referer`, `http_user_agent`, `approval_status`, creation_time) values ("'.$_SESSION['survey_id'].'", "'.$row_request['sno'].'", "'.$_SESSION['user_id'].'", "'.$_SESSION['user_type'].'", "'.$user['mobile_number'].'", "'.$otp.'", "'.$_SERVER['REMOTE_ADDR'].'", "'.$_SERVER['HTTP_REFERER'].'", "'.$_SERVER['HTTP_USER_AGENT'].'", "approve",  "'.date("Y-m-d H:i:s").'")';
		execute_query($sql);
		if(mysqli_error($db)){
			$data[] = array("status"=>"error", "msg"=>"VF#01 : Some error occured");
		}
		else{
			$data[] = array("status"=>"otp_sent", "msg"=>"OTP Sent on mobile.");
		}
...
Regards,
WeKnow Technologies';
			send_sms($user['mobile_number'],$get_msg, $template_id, $pe_id, $hindi='');
	}
}
elseif($id=='verify_otp_admin'){
	$msg='';
	//print_r($_POST);
	$sql = 'select * from survey_invoice_validation where survey_id="'.$_SESSION['survey_id'].'" and user_id="'.$_SESSION['user_id'].'" and user_type="'.$_SESSION['user_type'].'" and approval_status="approve" order by sno desc limit 1';	
	$result = execute_query($sql);
	if(mysqli_num_rows($result)!=0){
		$row = mysqli_fetch_assoc($result);
		if($row['otp_verify']==$_POST['otp']){
			$sql = 'update survey_invoice_validation set otp_verify=1 where sno="'.$row['sno'].'"';
			execute_query($sql);
			
			switch($_SESSION['user_type']){
				case 'ar':{
					$sql = 'update survey_invoice_validation set status=4 where sno="'.$row['request_id'].'"';
					execute_query($sql);

					$sql = 'update survey_invoice set approval_status="4" where sno="'.$_SESSION['survey_id'].'"';
					break;
				}
				case 'adco':{
					$sql = 'update survey_invoice_validation set status=3 where sno="'.$row['request_id'].'"';
					execute_query($sql);

					$sql = 'update survey_invoice set approval_status="3" where sno="'.$_SESSION['survey_id'].'"';
					break;
				}
				case 'ado':{
					$sql = 'update survey_invoice_validation set status=2 where sno="'.$row['request_id'].'"';
					execute_query($sql);

					$sql = 'update survey_invoice set approval_status="2" where sno="'.$_SESSION['survey_id'].'"';
					break;
				}
			}
			execute_query($sql);
			//echo mysqli_error($db).$sql;
			$data[] = array("status"=>"verified", "msg"=>"Form Verified and Forwarded to Next Level");
		}
		else{
			$data[] = array("status"=>"invalid", "msg"=>"Incorrect OTP");
		}
...
elseif($id=='form_review'){
	switch($_SESSION['user_type']){
		case 'ar':{
			$sql = 'select * from survey_invoice_validation where survey_id="'.$_SESSION['survey_id'].'" and approval_status="approve" and user_type="secretary" and status="3"';
			$row_request = mysqli_fetch_assoc(execute_query($sql));
		
			$sql = 'update survey_invoice_validation set status=2 where sno='.$row_request['sno'];
			execute_query($sql);
			...
			if(mysqli_error($db)){
				...
				$data[] = array("status"=>"error", "error"=>"FV02: Unable to save data.");
			}
		...
		case 'adco':{
			...
			$sql = 'update survey_invoice_validation set status=1 where sno='.$row_request['sno'];
			execute_query($sql);
		
			$sql = 'update survey_invoice set approval_status=1 where sno='.$_SESSION['survey_id'];
			execute_query($sql);
			if(mysqli_error($db)){
				//echo mysqli_error($db);
				$data[] = array("status"=>"error", "error"=>"FV02: Unable to save data.");
		...
		case 'ado':{
			...
			$sql = 'update survey_invoice_validation set status=5 where sno='.$row_request['sno'];
			execute_query($sql);
		
			$sql = 'update survey_invoice set approval_status=0 where sno='.$_SESSION['survey_id'];
			execute_query($sql);
			if(mysqli_error($db)){
				//echo mysqli_error($db);
				$data[] = array("status"=>"error", "error"=>"FV02: Unable to save data.");
	...
	//echo $sql;
	$user = mysqli_fetch_assoc(execute_query($sql));
	$sql = 'insert into survey_invoice_validation (survey_id, request_id, user_id, user_type, mobile_number, `ip_address`, `http_referer`, `http_user_agent`, `approval_status`, remarks, creation_time) values ("'.$_SESSION['survey_id'].'", "'.$row_request['sno'].'", "'.$_SESSION['user_id'].'", "'.$_SESSION['user_type'].'", "'.$user['mobile_number'].'", "'.$_SERVER['REMOTE_ADDR'].'", "'.$_SERVER['HTTP_REFERER'].'", "'.$_SERVER['HTTP_USER_AGENT'].'", "reject", "'.$_POST['reason'].'", "'.date("Y-m-d H:i:s").'")';
	execute_query($sql);
	if(mysqli_error($db)){
		$data[] = array("status"=>"error", "error"=>"FV01: Unable to save data.");
	}
	else{
		$data[] = array("status"=>"Update", "msg"=>"Data Saved");	
	}
}
elseif($id=='dist'){
