	...
	<!----------9th start-------------------------------------------------------->
												<div class="step">
													...
													<div class="col-sm-12">
														...
														<p class="small">Preview Date &amp; Time : <?php echo date("d-m-Y H:i:s"); ?></p>
														<?php
														$visible = 'none';
														if($_SESSION['user_type']=='ado' && $row_invoice['approval_status']==1){
															$visible = 'flex';
														}
														elseif($_SESSION['user_type']=='adco' && $row_invoice['approval_status']==2){
															$visible = 'flex';
														}
														elseif($_SESSION['user_type']=='ar' && $row_invoice['approval_status']==3){
															$visible = 'flex';
														}
														
														?>
														<div id="buttons" style="display: <?php echo $visible; ?>;">
															<button type="button" class="btn btn-info" onClick="$('#send_otp_button').show();">सत्यापित करें </button>
															<button type="button" class="btn btn-danger" onClick="form_review();">पुनः निरिक्षण के लिये भेजें</button>
														
															<div class="col-sm-12 form-group my-auto" id="send_otp_button" style="display: none">
																<button type="button" name="send_otp_btn" id="send_otp_btn" tabindex="<?php echo $tab++; ?>"  class="btn btn-info" onClick="send_verify_otp();">ओ.टी.पी. भेजे</button>
															</div>
															<div class="col-sm-12 form-group" id="otp_verify" style="display: none">
																<div class="row">
																	...
																	<div class="col-sm-8 form-group my-auto">
																		<button type="button" name="verify_otp_btn" id="verify_otp_btn" tabindex="<?php echo $tab++; ?>"  class="btn btn-info" onClick="verify_otp_admin($('#survey_id').val(), '', $('#user_otp').val());">वेरिफाई करें</button>
																		<button type="button" name="send_otp_btn" id="send_otp_btn" tabindex="<?php echo $tab++; ?>"  class="btn btn-info" onClick="send_verify_otp($('#survey_id').val(), '');">पुनः ओ.टी.पी. भेजे</button>
																	</div>
																</div>
															</div>
														</div>
...
function send_verify_otp(){
	var data = {"term":"b", "id":"send_verify_otp"};
	$.ajax({
        type: "POST",
        url: actionUrl,
...
// JavaScript Document
function verify_otp_admin(val, number, otp){
	var data = {"term":"b", "id":"verify_otp_admin", "otp":otp};
	$.ajax({
        type: "POST",
        url: actionUrl,
        data: data, // serializes the form's elements.
...
function form_review(){
	let text;
	let reason = prompt("कृप्या पुनः निरिक्षण के लिये भेजने कि वजह दर्ज करें", "");
	//console.log(reason);
	if (reason == null || reason == "") {
		if(reason == ""){
			alert('वजह दर्ज करना अनिवार्य है');
		}
		text = "User cancelled the prompt.";
	} 
	else {
		text = "Hello " + reason + "! How are you today?";
		var data = {"term":"b", "id":"form_review", "val":'', "reason":reason};
		$.ajax({
			type: "POST",
