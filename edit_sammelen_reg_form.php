	...
	if (isset($_POST['member_mob1'])) {
		...
		if (empty($msg)) {
			...
			$sql = "UPDATE sammelen_invoice SET  
				...
				`tehsil_id`= '{$tehname}',
				`block_id`= '{$blockname}', 
				`society_type_id`= '{$stype}', 
				`society_name_id`= '{$sname}',
				`invName`= '{$invname}',
				`invFatherName`= '{$invfather}', 
				`invMobno`= '{$invmobno}',
			...
			if (mysqli_error($db)) {
				...
				$del_sql = 'delete from sammelen_trans where sammelen_invoice_id="'.$_POST['eid'].'"';
				...
				for ($i = 1; $i <= $_POST['row_count']; $i++) {
					if ($_POST['member_name' . $i] != '') {
						...
						$sql = "INSERT INTO `sammelen_trans`( `sammelen_invoice_id`, `name`, `father_name`, `mobile_num`,`tehseelId`, `blociId`,`memberType`,`creation_time`,`created_by` ) VALUES ('".$_POST['eid']."','$name','$fname','$mobno','$teh','$block','$memberType','" . date("d-m-Y H:i:s") . "','" . $_SESSION['username'] . "')";
...
if(isset($_GET['eid'])){
    ...
    $_POST['society_name'] = $invoice['society_name_id'];
	...
	$_POST['division_name'] = $invoice['mandal_id'];
    $_POST['district_name'] = $invoice['district_id'];
    $_POST['tehseel_name'] = $invoice['tehsil_id'];
    $_POST['block_name'] = $invoice['block_id'];
                    ...
                    <div class="row d-flex my-auto">
                        <div class="col-md-12">
                            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data" >
                                ...
                                <div class="col-sm-12">
                                    <div class="row">	
                                        ...
                                        <div class="col-md-3">
                                                ...
                                                <input type="text" name="invname" id="invname" class="form-control" value="<?php echo $_POST['invname']; ?>" required>
                                        </div>
                                        <div class="col-md-2">
                                            <label for="invfather">सम्मेलन समिति संयोजक  के पिता का नाम *</label>
                                            <input type="text" name="invfather" id="invfather" class="form-control" value="<?php echo $_POST['invfather']; ?>" required>
                                        </div>
                                        <div class="col-md-2">
                                                <label for="invAddress">सम्मेलन समिति संयोजक  का पता *</label>
                                                <input type="text" name="invAddress" id="invAddress" class="form-control" value="<?php echo $_POST['invAddress']; ?>" required>
                                        </div>
                                        <div class="col-md-2">
                                            <label for="invmobno">सम्मेलन समिति संयोजक का मोबाईल नंबर*</label>
                                            <input type="number" pattern="[0-9]{10}" name="invmobno" id="invmobno" class="form-control" value="<?php echo $_POST['invmobno']; ?>" required>
                                        </div>
                                    </div>
                                    ...
                                    <?php echo $i; ?>.
                                        ...
                                        <div class="col-md-2">
                                            ...
                                            <label>पिता का नाम  </label>
                                                <input type="text" name="member_father_name<?php echo $i; ?>" id="member_father_name<?php echo $i; ?>" class="form-control" placeholder="" value="<?php echo $_POST['member_father_name'.$i]; ?>"  required>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <label>मोबाईल नंबर </label>
                                                <input type="number" pattern="[0-9]{10}" name="member_mob<?php echo $i; ?>" id="member_mob<?php echo $i; ?>" class="form-control" placeholder="" value="<?php echo $_POST['member_mob'.$i]; ?>" required>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <label>तहसील</label>
                                                <select class="form-control" name="member_tehseel<?php echo $i; ?>" id="member_tehseel<?php echo $i; ?>"  required onChange="member_fill_block_name(this.value, <?php echo $i; ?>);" required>
                                ...
                                <button  class="btn btn-success" id="update" type="submit" name="update">Update (प्रेषित करें)</button>
								<input type="hidden" name="eid" id="eid" value="<?php echo $_POST['eid']; ?>">
                                </div>
                            </form>
                    </div>
                </div>
...
<script>
    ...
    function add_rows(){
		...
		var txt = '<div class="row border rounded m-2 p-2 border-secondary" id="add_rows_length">'+id+'<div class="col-md-2"><div class="form-group"><label>प्रतिभागी का नाम </label><br><input type="text" name="member_name'+id+'" id="member_name'+id+'" class="form-control" placeholder="" value="" tabindex="" required ></div></div><div class="col-md-2"><div class="form-group"><label >पिता का नाम  </label><input type="text" name="member_father_name'+id+'" id="member_father_name'+id+'"  class="form-control" placeholder=" "value="" tabindex="" required></div></div><div class="col-md-2"><div class="form-group"><label >मोबाईल नंबर </label><br><input type="number" pattern="[0-9]{10}" name="member_mob'+id+'" id="member_mob'+id+'" class="form-control" placeholder="" value="" tabindex="" required ></div></div><div class="col-md-2"><div class="form-group"><label>तहसील</label><select class="form-control" name="member_tehseel'+id+'" id="member_tehseel'+id+'"  onChange="member_fill_block_name(this.value,'+id+');" required></select></div></div><div class="col-md-2" ><div class="form-group"><label>विकासखंड</label><select class="form-control" name="member_block'+id+'" id="member_block'+id+'"  required></select></div></div><div class="col-md-2"><div class="form-group"><select name="memberType'+id+'" id="memberType'+id+'"  class="form-control" required ><option value="" selected disabled>--SELECT--</option><option value="अध्यक्ष">अध्यक्ष</option><option value="उपाअध्यक्ष">उपाअध्यक्ष</option><option value="प्रबंध समिति संचालक">प्रबंध समिति संचालक</option><option value="संस्था के प्रतिनिधि/डेलीगेट">संस्था के प्रतिनिधि/डेलीगेट </option><option value="सदस्य">सदस्य</option><option value="अन्य">अन्य</option></select></div></div><div class="col-md-1 d-flex justify-content- align-items-center"><button type="button" class="btn btn-info pull-right" onClick="add_rows()">Add [ + ]</button></div></div>';
		$("#add_wrap").append(txt);
...
function member_fill_block_name(val,idforappend, selected=''){
	var data = {term:"b", id:"blockname", val:val};
	$.ajax({
        type: "POST",
        url: actionUrl,
...
<script>	
    ...
    $(document).ready(function() {
		...
		fill_tehseel_name(<?php echo $_POST['district_name']; ?>,<?php echo $_POST['member_tehseel1']; ?>);
		member_fill_block_name(<?php echo $_POST['member_tehseel1']; ?>, 1, <?php echo $_POST['member_block1']; ?>);
		<?php
		for($i=2;$i<=$_POST['row_count'];$i++){
			echo 'fill_tehseel_name('.$_POST['district_name'].','.$_POST['member_tehseel'.$i].', '.$i.');';	
			echo 'member_fill_block_name('.$_POST['member_tehseel'.$i].', '.$i.', '.$_POST['member_block'.$i].');';
		}
	  	?>
